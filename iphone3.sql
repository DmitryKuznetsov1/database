...
CREATE TABLE available_color (
    ...
    CONSTRAINT available_color_color_id_check CHECK ((color_id > 0)),
    CONSTRAINT available_color_model_id_check CHECK ((model_id > 0))
);
...
CREATE TABLE available_storage (
    ...
    CONSTRAINT available_storage_model_id_check CHECK ((model_id > 0)),
    CONSTRAINT available_storage_storage_id_check CHECK ((storage_id > 0))
);
...
CREATE TABLE cpu (
    ...
    CONSTRAINT cpu_cpu_manufacturer_id_check CHECK ((cpu_manufacturer_id > 0)),
    CONSTRAINT cpu_frequency_ghz_check CHECK ((frequency_ghz > (0)::double precision)),
    CONSTRAINT cpu_l2_cache_mb_check CHECK ((l2_cache_mb > (0)::double precision)),
    CONSTRAINT cpu_number_of_cores_check CHECK ((number_of_cores > 0))
);
...
CREATE TABLE cpu_manufacturer (
    ...
    CONSTRAINT cpu_manufacturer_year_of_foundation_check CHECK ((year_of_foundation > 0))
);
...
CREATE TABLE display (
    ...
    CONSTRAINT display_pixels_per_inch_check CHECK ((pixels_per_inch > 0)),
    CONSTRAINT display_screen_size_inch_check CHECK ((screen_size_inch > (0)::double precision))
);
...
CREATE TABLE model (
    ...
    cpu_id integer NOT NULL,
    display_id integer NOT NULL,
    CONSTRAINT model_battery_capacity_mah_check CHECK ((battery_capacity_mah > 0)),
    CONSTRAINT model_cpu_id_check CHECK ((cpu_id > 0)),
    CONSTRAINT model_display_id_check CHECK ((display_id > 0)),
    CONSTRAINT model_memory_gb_check CHECK ((memory_gb > (0)::double precision))
);
...
CREATE TABLE storage (
    ...
    CONSTRAINT storage_storage_gb_check CHECK ((storage_gb > 0))
);
...
SELECT pg_catalog.setval('color_color_id_seq', 12, true);
...
--
-- Name: available_color available_color_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY available_color
    ADD CONSTRAINT available_color_pkey PRIMARY KEY (model_id, color_id);


--
-- Name: available_storage available_storage_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY available_storage
    ADD CONSTRAINT available_storage_pkey PRIMARY KEY (model_id, storage_id);
